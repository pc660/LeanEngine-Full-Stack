...
/***/ function(module, exports) {
	...
	exports["default"] = ["SweetAlert", "$state", "$log", "$scope", "$document", "$timeout", "lcConfig", "$window", "providerFac", "fileFac", "Upload", function (SweetAlert, $state, $log, $scope, $document, $timeout, lcConfig, $window, providerFac, fileFac, Upload) {
	  "ngInject";
...
/***/ function(module, exports) {
	...
	exports["default"] = ["$rootScope", "authFac", "$log", "$state", "$scope", "$uibModal", "userFac", "lcConfig", "$window", "productFac", "orderFac", "formConfig", "providerFac", "SweetAlert", function ($rootScope, authFac, $log, $state, $scope, $uibModal, userFac, lcConfig, $window, productFac, orderFac, formConfig, providerFac, SweetAlert) {
	  "ngInject";
	  $scope.admin = false;
...
/***/ function(module, exports) {
	...
	exports["default"] = ["$sce", "$log", "$scope", "$window", "$stateParams", "productFac", "providerFac", "multiChoiceConfig", function ($sce, $log, $scope, $window, $stateParams, productFac, providerFac, multiChoiceConfig) {
	  ...
	  $scope.setStorageContent = function (date) {
...
/***/ function(module, exports) {
	...
	exports["default"] = ["SweetAlert", "lcConfig", "$location", "$scope", "$window", "$log", "commonSer", "helloSer", "authFac", function (SweetAlert, lcConfig, $location, $scope, $window, $log, commonSer, helloSer, authFac) {
	  "ngInject";
...
/***/ function(module, exports) {
	...
	exports['default'] = ["$scope", "$document", "$state", "lcConfig", "$window", "fileFac", function ($scope, $document, $state, lcConfig, $window, fileFac) {
	  'ngInject';
	}];
...
/***/ function(module, exports) {
	...
	exports['default'] = ["SweetAlert", "$log", "$scope", "$state", "$window", "$sce", "$uibModal", "orderFac", "productFac", "userFac", "lcConfig", function (SweetAlert, $log, $scope, $state, $window, $sce, $uibModal, orderFac, productFac, userFac, lcConfig) {
	  'ngInject';
	  ...
	  $scope.confirmRevoke = function (orderId) {
	    $scope.cancelOrder(orderId);
	  };

	  $scope.verifyUnpaid = function (orderId) {
	  ...
	  $scope.verifyPaid = function (orderId) {
...
$templateCache.put("app/auth/home/home.html","<nav class=\"navbar navbar-inverse\" role=\"navigation\"><div class=\"navbar-header\"><a class=\"navbar-brand\" ui-sref=\"#\">中途港产品发布系统</a></div><ul class=\"nav navbar-nav\"><li><a ui-sref=\".add-product\">发布产品</a></li><li ng-if=\"admin\"><a ui-sref=\".show-products\">产品管理</a></li><li ng-if=\"admin\"><a ui-sref=\".show-providers\">供应商管理</a></li><li ng-if=\"admin\"><a ui-sref=\".show-orders\">订单管理</a></li><li ng-if=\"provider\"><a ui-sref=\".show-orders\">订单管理</a></li><li ng-if=\"organizer\"><a ui-sref=\".show-orders\">订单审核</a></li><li ng-if=\"admin\"><a ui-sref=\".add-provider-account\">添加账号</a></li><li ng-class=\"{\'unfinished\':unfinished}\"><span class=\"dot\"></span> <a ui-sref=\".my-account\">我的账号</a></li><li><a ng-click=\"logout();\">退出登录</a></li></ul></nav><div ui-view=\"\"></div>");
...
$templateCache.put("app/common/sale/order/add_order.html","<md-tabs md-dynamic-height=\"\" md-border-bottom=\"\"><md-tab label=\"提交订单\"><table class=\"order-table\"><tr><td colspan=\"99\" class=\"title\">产品基本信息</td></tr><tr><td>产品编号</td><td>{{ product.objectId }}</td></tr><tr><td>出发地</td><td><span ng-if=\"product.start.city == \'市辖区\'\">{{ product.start.province }}</span> <span ng-if=\"product.start.city != \'市辖区\'\">{{ product.start.city }}</span></td></tr><tr><td>产品名称</td><td>{{ product.productName }}</td></tr><tr><td>出团日期</td><td>{{ order.date }}</td></tr><tr><td>供应商</td><td>{{ product.provider.companyname }}</td></tr><tr><td colspan=\"99\" class=\"title\">团费</td></tr><tr><td>类型</td><td>数量</td><td>单价</td><td>小计</td></tr><tr><td>成人</td><td><counter ng-model=\"order.adult\"></counter></td><td>{{ order.adultCompanySalePrice }}</td><td></td></tr><tr><td>儿童</td><td><counter ng-model=\"order.child\"></counter></td><td>{{ order.childCompanySalePrice }}</td><td>{{ order.totalPrice }}</td></tr><tr><td colspan=\"99\" class=\"title\">预定信息</td></tr><tr><td>业务联系人</td><td><input type=\"text\" ng-model=\"order.contactname\"></td></tr><tr><td>手机</td><td><input type=\"text\" ng-model=\"order.cellphone\"></td></tr><tr><td>邮箱</td><td><input type=\"text\" ng-model=\"order.email\"></td></tr></table></md-tab><md-tab label=\"游客信息\"><md-content><table class=\"customers\"><tr><th>姓名</th><th>类型</th><th>性别</th><th>证件类型</th><th>证件号码</th><th>手机</th><th>出生日期</th><th>备注</th></tr><tr ng-repeat=\"customer in order.customers\"><td><input type=\"text\" ng-model=\"customer.name\"></td><td><md-input-container><md-select ng-model=\"customer.type\"><md-option ng-selected=\"$first\" ng-value=\"type\" ng-repeat=\"type in types\">{{ type }}</md-option></md-select></md-input-container></td><td><md-input-container><md-select ng-model=\"customer.sex\"><md-option ng-selected=\"$first\" ng-value=\"sex\" ng-repeat=\"sex in sexList\">{{ sex }}</md-option></md-select></md-input-container></td><td><md-input-container><md-select ng-model=\"customer.license\"><md-option ng-selected=\"$first\" ng-value=\"license\" ng-repeat=\"license in licenseList\">{{ license }}</md-option></md-select></md-input-container></td><td><input type=\"text\" ng-model=\"customer.licenseNumber\"></td><td><input type=\"text\" ng-model=\"customer.cellphone\"></td><td class=\"birthDate\"><input ng-model=\"customer.birthDate\" type=\"text\" data-date-type=\"string\" data-model-date-format=\"mediumDate\" bs-datepicker=\"\" data-date-format=\"mediumDate\" container=\"body\" data-autoclose=\"true\"></td><td class=\"extra\"><div><input type=\"checkbox\" ng-model=\"customer.needVisa\"> 需要办签</div><div><input type=\"checkbox\" ng-model=\"customer.needShare\"> 需要拼房</div></td></tr></table></md-content><div class=\"center\"><md-button class=\"md-raised md-primary\" ng-click=\"submitOrder();\">提交订单</md-button></div></md-tab></md-tabs>");
$templateCache.put("app/common/sale/order/show_order.html","<div class=\"show-order\" layout=\"row\"><md-sidenav class=\"md-sidenav-order\" md-component-id=\"show-order\" md-is-locked-open=\"true\" md-whiteframe=\"2\"><md-toolbar class=\"md-theme-indigo\"><h1 class=\"md-toolbar-tools\">订单管理</h1></md-toolbar><div class=\"sidebar-button\"><md-button ng-click=\"allOrder()\" class=\"md-primary\">全部订单</md-button><md-button ng-click=\"unpaidOrderUnverified()\" class=\"md-primary\">待付款订单(等待确认)</md-button><md-button ng-click=\"unpaidOrderVerified()\" class=\"md-primary\">待付款订单(已确认)</md-button><md-button ng-click=\"paidOrder()\" class=\"md-primary\">已付款订单(等待确认)</md-button><md-button ng-click=\"finishedOrder()\" class=\"md-primary\">已完成订单</md-button><md-button ng-if=\"admin\" ng-click=\"getRevoke()\" class=\"md-primary\">申请退款订单</md-button></div></md-sidenav><md-content layout-padding=\"\"><table class=\"order\"><tbody ng-repeat=\"order in orders\"><tr><td colspan=\"99\"><div class=\"order-title\"><span>订单号: {{order.objectId}}</span> <span>下单时间: <span ng-bind=\"order.createdAt | date:\'yyyy-MM-dd HH:mm\'\"></span></span> <span>供应商: 上海旅行社</span> <a ng-click=\"showContact(order);\">和我联系</a></div></td></tr><tr><td><div><div>订单来源： {{ order.contactname }}</div><a><span ng-repeat=\"prefix in order.product.prefixArray track by $index\">[{{ prefix }}]</span> {{ order.product.fullName }}</a></div></td><td><div class=\"detail\"><p><strong>￥{{ order.totalPrice }}</strong></p><div>成人：{{ order.adult }}人 儿童：{{ order.child }}人</div><div>出团时间: {{ order.startDate }}</div></div></td><td><div class=\"status\"><p><strong ng-if=\"order.status == 1\">未付款(等待确认)</strong> <strong ng-if=\"order.status == 2\">未付款(已确认)</strong> <strong ng-if=\"order.status == 3\">已付款(等待确认)</strong> <strong ng-if=\"order.status == 4\">已完成</strong> <strong ng-if=\"order.status == 5\">申请退款</strong> <strong ng-if=\"order.status == 6\">申请取消</strong></p><md-button ng-if=\"(admin || organizer) && order.status == 1\" ng-click=\"verifyUnpaid(order.objectId);\" class=\"md-raised md-primary\">审核通过</md-button><md-button ng-if=\"(!admin && !organizer) && order.status == 2\" ng-click=\"paid(order.objectId);\" class=\"md-raised md-primary\">确认付款</md-button><md-button ng-if=\"(admin || organizer) && order.status == 3\" ng-click=\"verifyPaid(order.objectId);\" class=\"md-raised md-primary\">确认交易完成</md-button><md-button ng-if=\"(admin || organizer) && order.status == 5\" ng-click=\"confirmRevoke(order.objectId);\" class=\"md-raised md-primary\">确认退款</md-button><md-button ng-if=\"(admin || organizer) && order.status == 6\" ng-click=\"confirmRevoke(order.objectId);\" class=\"md-raised md-primary\">确认取消订单</md-button></div></td><td><div class=\"operation\"><a ng-if=\"!admin\" target=\"_blank\" ui-sref=\"sale.show-order-detail({orderId: \'{{order.objectId}}\' })\" ui-sref-opts=\"{ absolute: true }\">显示详情</a> <a ng-if=\"admin\" target=\"_blank\" ui-sref=\"home.show-order-detail({orderId: \'{{order.objectId}}\' })\" ui-sref-opts=\"{ absolute: true }\">显示详情</a> <a href=\"{{ order.fileUrl }}\" download=\"\">查看确认单</a> <a ng-if=\"!admin\" ng-click=\"printReceipt();\">打印发票</a> <a ng-if=\"!admin && order.status == 1\" ng-click=\"cancelOrder(order.objectId);\">取消订单</a> <a ng-if=\"!admin && order.status == 2\" ng-click=\"applyCancel(order.objectId);\">申请取消订单</a> <a ng-if=\"!admin && (order.status == 3 || order.status == 4 || order.status == 5)\" ng-click=\"applyRevoke(order.objectId);\">申请退款</a></div></td></tr></tbody></table></md-content></div>");
$templateCache.put("app/common/sale/order/show_order_detail.html","<table><tr><td colspan=\"99\" class=\"title\">预订信息</td></tr><tr><td>联系人</td><td colspan=\"99\">{{ order.contactname }}</td></tr><tr><td>联系方式</td><td colspan=\"99\">{{ order.cellphone }}</td></tr><tr><td>邮件</td><td colspan=\"99\">{{ order.email }}</td></tr><tr><td colspan=\"99\" class=\"title\">产品信息</td></tr><tr><td>产品编号</td><td>产品名称</td><td>出发城市</td><td>出游人数</td><td>订单金额</td></tr><tr><td>{{ order.objectId }}</td><td><a><span ng-repeat=\"prefix in order.product.prefixArray\">[{{ prefix }}]</span> {{ order.product.fullName }}</a><div>供应商: {{ order.product.companyname }}</div></td><td>上海</td><td><span>成人: {{ order.adult }}人</span> <span>/</span> <span>儿童: {{ order.child }}人</span></td><td>{{ order.totalPrice }}</td></tr><tr><td colspan=\"99\" class=\"title\">游客信息</td></tr><tr><td>姓名</td><td>客户类型</td><td>证件类型</td><td>证件号码</td><td>性别</td><td>出生日期</td><td>备注信息</td></tr><tr ng-repeat=\"customer in order.customers\"><td><span>{{ customer.contactname }}</span> <span>手机：{{ customer.cellphone }}</span></td><td>{{ customer.type }}</td><td>{{ customer.license }}</td><td>{{ customer.licenseNumber }}</td><td>{{ customer.sex }}</td><td>{{ customer.birthDate }}</td><td><p ng-if=\"customer.needShare\">需要拼房</p><p ng-if=\"customer.needVisa\">需要签证</p></td></tr></table>");
$templateCache.put("app/common/sale/search/search.html","<div class=\"selected-tags\" ng-if=\"selectedTags\"><div ng-repeat=\"(key, value) in selectedTags\"><span class=\"key-span\">{{ key }}:</span> <span class=\"value-span\">{{ value }}</span> <a class=\"cancel-icon\" ng-click=\"removeTag(key);\"></a></div></div><table class=\"search-option\"><tr><td>出发城市</td><td class=\"address-tag\"><div><span ng-class=\"{\'selected-tag\': !showAddress}\" ng-click=\"showAddress = false\" class=\"unlimited\">不限</span> <span ng-class=\"{\'selected-tag\': showAddress}\" ng-click=\"showAddress = true\" class=\"unlimited\">选择城市</span><address ng-show=\"showAddress\" class=\"address\" show-county=\"false\" is-editing=\"true\" ng-model=\"start\"></address></div></td></tr><tr><td>出发日期</td><td><span ng-class=\"{\'selected-tag\': !showDate}\" ng-click=\"showDate = false\" class=\"unlimited\">不限</span> <span ng-class=\"{\'selected-tag\': showDate}\" ng-click=\"showDate = true\" class=\"unlimited\">选择出发日期</span><div ng-show=\"showDate\" class=\"date-wrapper\" layout=\"row\"><input ng-model=\"startDate\" type=\"text\" data-date-type=\"string\" data-model-date-format=\"mediumDate\" bs-datepicker=\"\" data-date-format=\"mediumDate\" data-autoclose=\"true\"><div class=\"separator\">-</div><input ng-model=\"endDate\" type=\"text\" data-date-type=\"string\" data-model-date-format=\"mediumDate\" bs-datepicker=\"\" data-date-format=\"mediumDate\" data-autoclose=\"true\"></div></td></tr><tr><td>行程天数</td><td class=\"selectable\"><div class=\"stop-day\"><span class=\"unlimited\" ng-class=\"{\'selected-tag\': index[\'行程天数\'] == -1}\" ng-click=\"pick(\'行程天数\', -1)\">不限</span> <span class=\"unlimited\" ng-class=\"{\'selected-tag\': $index==index[\'行程天数\']}\" ng-click=\"pick(\'行程天数\', $index)\" ng-repeat=\"day in tags[\'行程天数\']\">{{ day }}天</span></div></td></tr><tr><td>供应商</td><td><div class=\"selectable\"><span class=\"unlimited\" ng-class=\"{\'selected-tag\': index[\'供应商\'] == -1}\" ng-click=\"pick(\'供应商\', -1)\">不限</span> <span class=\"unlimited\" ng-class=\"{\'selected-tag\': $index==index[\'供应商\']}\" ng-click=\"pick(\'供应商\', $index)\" ng-repeat=\"provider in tags[\'供应商\'] track by $index\">{{ provider }}</span></div></td></tr><tr class=\"selectable\"><td>酒店星级</td><td><span class=\"unlimited\" ng-class=\"{\'selected-tag\': index[\'酒店标准\'] == -1}\" ng-click=\"pick(\'酒店标准\', -1)\">不限</span> <span class=\"unlimited\" ng-class=\"{\'selected-tag\': $index==index[\'酒店标准\']}\" ng-click=\"pick(\'酒店标准\', $index)\" ng-repeat=\"hotel in tags[\'酒店标准\']\">{{ hotel }}</span></td></tr><tr class=\"selectable\"><td>出行方式</td><td><span class=\"unlimited\" ng-class=\"{\'selected-tag\': index[\'交通方式\'] == -1}\" ng-click=\"pick(\'交通方式\', -1)\">不限</span> <span class=\"unlimited\" ng-class=\"{\'selected-tag\': $index==index[\'交通方式\']}\" ng-click=\"pick(\'交通方式\', $index)\" ng-repeat=\"transport in tags[\'交通方式\']\">{{ transport }}</span></td></tr><tr class=\"selectable\"><td>类型</td><td><span class=\"unlimited\" ng-class=\"{\'selected-tag\': index[\'类型\'] == -1}\" ng-click=\"pick(\'类型\', -1)\">不限</span> <span class=\"unlimited\" ng-class=\"{\'selected-tag\': $index==index[\'类型\']}\" ng-click=\"pick(\'类型\', $index)\" ng-repeat=\"type in tags[\'类型\']\">{{ type }}</span></td></tr><tr><td>目的地</td><td><div class=\"keyword\"><input type=\"text\" placeholder=\"请输入目的地关键词\" ng-model=\"destination\"><md-button class=\"md-raised md-primary\" ng-click=\"search();\">确认搜索</md-button></div></td></tr></table><div class=\"products\"><div class=\"product\" ng-repeat=\"product in products\"><div class=\"left\"><a class=\"product-title\" target=\"_blank\" ui-sref=\"sale.show-product({productId: \'{{product.objectId}}\' })\" ui-sref-opts=\"{ absolute: true }\"><span ng-repeat=\"prefix in product.prefixArray track by $index\"><span>[</span> <span>{{ prefix }}</span> <span>]</span></span> <span>{{product.productName}}</span></a><div class=\"metadata\"><span>出发地: {{ product.start.city }}</span> <span>行程天数：{{ product.duration }}天{{ product.hotelDuration }}晚</span> <span>最近班期： <em>{{ product.latestDate }}</em></span></div><div class=\"metadata\"><span>线路编号：{{ product.objectId }}</span> <span>供应商：{{ product.provider.companyname }}</span></div></div><div class=\"right\"><div class=\"pricebox\"><span>门市价 <em><span class=\"large\">{{ product.latestAdultCompanySalePrice }}</span></em></span> <span>同行价 <em><span class=\"large\">{{ product.latestAdultCompanyCompetitorPrice }}</span></em></span></div><div><a href=\"{{ product.fileUrl }}\"><md-button class=\"md-raised md-primary\">打印行程单</md-button></a></div></div></div></div>");
