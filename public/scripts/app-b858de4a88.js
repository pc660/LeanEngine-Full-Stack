...
/***/ function(module, exports, __webpack_require__) {
	...
	*/
	...
	angular.module('webProject', ['ngAnimate', 'ngCookies', 'ui.router', 'ngMaterial', 'base64', 'mgcrea.ngStrap', 'ngFileUpload', 'angular-md5', 'ui.bootstrap', 'monospaced.elastic', 'angucomplete-alt', 'textAngular', 'ng.ueditor', 'materialCalendar', 'oitozero.ngSweetAlert', 'monospaced.elastic'])
	// 配置全局常量
...
/***/ function(module, exports) {
	...
	exports['default'] = ["$log", "$rootScope", "$mdSidenav", "$state", "$window", "providerFac", "$uibModal", "productFac", function ($log, $rootScope, $mdSidenav, $state, $window, providerFac, $uibModal, productFac) {
	  'ngInject';
	  return {
	    restrict: 'E',
	    ...
	    link: function link(scope, element, attr) {
	      ...
	      * */
	      scope.doSearch = function (searchString) {
	        var query = JSON.stringify({ 'mainBusiness': searchString, 'type': 'contain' });
	      ...
	      * */
	      scope.searchFromKeyword = function (keyword) {
...
/***/ function(module, exports) {
	...
	exports["default"] = ["$log", "$scope", "$stateParams", "providerFac", "$document", "$timeout", "lcConfig", "$window", "fileFac", function ($log, $scope, $stateParams, providerFac, $document, $timeout, lcConfig, $window, fileFac) {
	  "ngInject";
	  $scope.providers = [];
	  var PER_PAGE = 50;
...
/***/ function(module, exports) {
	...
	exports['default'] = ["authFac", "$log", "$state", "$scope", "$uibModal", "userFac", "lcConfig", "$window", "productFac", "formConfig", "providerFac", "SweetAlert", function (authFac, $log, $state, $scope, $uibModal, userFac, lcConfig, $window, productFac, formConfig, providerFac, SweetAlert) {
	  'ngInject';
...
/***/ function(module, exports) {
	...
	exports["default"] = ["SweetAlert", "lcConfig", "$location", "$scope", "$window", "$log", "commonSer", "helloSer", "authFac", function (SweetAlert, lcConfig, $location, $scope, $window, $log, commonSer, helloSer, authFac) {
	  ...
	  $scope.levels = [{ level: 2, value: "供应商" }, { level: 1, value: "分销商" }];
...
/***/ function(module, exports) {
	...
	exports['default'] = ["$rootScope", "$http", "$state", "lcConfig", "$window", "$log", "$cookies", "$base64", function ($rootScope, $http, $state, lcConfig, $window, $log, $cookies, $base64) {
	  ...
	  function register(username, password, level) {
	    $log.log(level);
	    return $http.post('/api/auth/register', { username: username, password: password, level: level });
	  }
...
$templateCache.put("app/common/directives/form/form.html","<div><div class=\"modal-header\"><h3 class=\"modal-title\">{{ title }}</h3></div><div class=\"modal-body\"><ul><li ng-repeat=\"item in items\"><div class=\"item\"><div class=\"key\">{{ item.key }}</div><div class=\"value\" ng-switch=\"\" on=\"item.type\"><input ng-switch-when=\"number\" type=\"number\" ng-model=\"item.value\"> <input class=\"datepicker\" ng-model=\"item.value\" ng-switch-when=\"date\" type=\"text\" data-date-type=\"string\" data-model-date-format=\"mediumDate\" bs-datepicker=\"\" data-date-format=\"mediumDate\" data-autoclose=\"true\"> <input class=\"datepicker\" ng-model=\"item.value\" ng-switch-when=\"text\" type=\"text\"></div></div></li></ul></div><div class=\"modal-footer\"><button class=\"btn btn-primary\" type=\"button\" ng-click=\"close()\">确认</button> <button class=\"btn btn-warning\" type=\"button\" ng-click=\"cancel()\">取消</button></div></div>");
